...
mentre per quanto riguarda l'algoritmo _PeriodSmart_ la complessità è risultata lineare.
...
Nel primo grafico si osserva come l'algoritmo _PeriodSmart_ risulti essere molto performante rispetto al suo rivale. <br />
Nel grafico in scala doppiamente logaritmica, invece si osserva come la curva dell'algortimo _PeriodNaive_ abbia pendenza doppia rispetto alla
curva dell'algortimo _PeriodSmart_, ciò è indicativo di una complessità rispettivamente quadratica e lineare, come evidenziato dall'analisi di complessità
svolta a lezione. <br />
...
In questo contesto il metodo di generazione prevede la scelta del periodo in modo pseudo-casuale, ciò rende le curve molto frastagliate, soprattutto per l'algortimo _PeriodNaive_. <br />
Tale andamento mette in evidenza come _PeriodNaive_ termini anticipatamente l'esecuzione nel caso di un periodo più contenuto della stringa, mentre ciò accade in maniera meno evidente nel caso dell'algoritmo
_PeriodSmart_. <br />
...
Rispetto al metodo 2, si prevede che l'ultimo carattere che compone il periodo, sia diverso da tutti quelli precedentemente generati. <br />
...
Mentre per quanto riguarda la controparte _PeriodSmart_ si nota come non risenta del caso pessimo ottendendo dei valori in linea con quelli ottenuti con i metodi precedenti. <br />
<br />
...
Come si evince dal grafico, il periodo tende ad essere prossimo alla lunghezza della stringa, questo verosimilmente perché in questo caso, dove l'alfabeto è composta da due caratteri
(a, b), si ha che la probabilità di avere sequenze ripetute di caratteri è molto bassa, di conseguenza il periodo tende ad essere molto alto. <br />
...
Il secondo metodo prevede la generazione randomica di un indice intero _q_ che verrà utilizzato per generare la sotto stringa da _[1, q]_ in seguito 
si procederà ripetendo tale sotto stringa fino a che la lunghezza _n_ sarà soddisfatta. <br />
La distribuzione risulta essere ben distribuita su tutto l'intervallo, questo perché la scelta di _q_ ha una probabilità di _$\frac{1}{n}$_ dove _n_ nel nostro caso è 100. <br />
...
```
<br />
Questo è una variante del secondo metodo, e rispetto a quest'ultimo la distribuzione delle occorrenze risulta essere molto più uniforme. <br />
...
Prevede la costruzione deterministica di una stringa che possa fungere da caso pessimo, si nota in maniera evidente
che tutte le stringhe generate hanno come periodo la lunghezza _n_ (100 nel nostro caso).
